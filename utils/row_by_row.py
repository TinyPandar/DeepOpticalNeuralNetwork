...
from batch_fista import batched_fista_amp_l1
...
def run_simulation():
    ...
    results = {
        ...
        'focus': {'wf': [], 'wf_reg': [], 'ggs1': [], 'ggs2_1': [], 'fista': [], 'pr_vbem': [], 'raf21_tm': []},
        'last_trial': None
    }
        ...
        for trial_idx in tqdm(range(NUM_TRIALS), desc="独立试验进度"):
            ...
            for batch_idx in range(0, N_ROWS, BATCH_SIZE):
                ...
                if ENABLE.get('fista', False):
                    ...
                    X_final_fista, iters_fista = batched_fista_amp_l1(P, I, FISTA_LAMBDA, a0, step=None, max_iter=FISTA_MAX_ITERS, tol=FISTA_TOL, verbose=False, device=device)
                    t1 = time.perf_counter()
                    X_align_fista = align_global_phase(X_final_fista, X_true)
                    acc_fista = correlation_coefficient(X_align_fista, X_true)
                    focus_fista = phase_conjugation_contrast(X_align_fista, X_true)
                    
                    fista_iters_trial.append(iters_fista)
                    fista_accs_trial.extend(acc_fista.cpu().numpy())
                    fista_focus_trial.append(focus_fista.cpu().numpy().mean())
                    fista_times_trial.append(t1 - t0)
                # 运行 Wirtinger Flow
...
def plot_focus(results):
    ...
    wf_mean, wf_std = stats(results['focus']['wf']) if ENABLE.get('wf', False) else None
    wf_reg_mean, wf_reg_std = stats(results['focus']['wf_reg']) if ENABLE.get('wf_reg', False) else None
    ggs1_mean, ggs1_std = stats(results['focus']['ggs1']) if ENABLE.get('ggs1', False) else None
    ggs2_1_mean, ggs2_1_std = stats(results['focus']['ggs2_1']) if ENABLE.get('ggs2_1', False) else None
    fista_mean, fista_std = stats(results['focus']['fista']) if ENABLE.get('fista', False) else None
    pr_vbem_mean, pr_vbem_std = stats(results['focus']['pr_vbem']) if ENABLE.get('pr_vbem', False) else None
    raf21_tm_mean, raf21_tm_std = stats(results['focus']['raf21_tm']) if ENABLE.get('raf21_tm', False) else None
